import { EXT_STRUCTURAL_METADATA, collectPropertyTableBufferViews, getStructuralMetadataData } from './extensions/ExtStructuralMetadata.js';
import { defined, extend, getTypedArrayCtor, isNumber, isString, readInterleavedArray } from './common/Util.js';
...
export default class GLTFLoader {
    ...
    load(options) {
        ...
        return Promise.all([gltf, animations, textures, extensions]).then(fullfilled => {
            ...
            // 加载 EXT_mesh_features 和 EXT_structural_metadata 扩展
            return this._loadExtensionsData(fullfilled[0]).then(() => fullfilled[0]);
        });
    }
    ...
    _loadPrimitive(primJSON, options) {
        ...
        return promise.then(assets => {
            ...
            // 保存原始扩展信息，用于后续解析 EXT_mesh_features 和 EXT_structural_metadata
    ...
    /**
     * 加载 EXT_mesh_features 和 EXT_structural_metadata 扩展数据
     * 只准备数据，不实例化插件（插件在主线程实例化）
     */
    _loadExtensionsData(gltf) {
        const extensionsUsed = this.gltf.extensionsUsed || [];
        const hasMeshFeatures = extensionsUsed.includes(EXT_MESH_FEATURES);
        const hasStructuralMetadata = extensionsUsed.includes(EXT_STRUCTURAL_METADATA);
        ...
        // 加载 structural metadata 所需的 buffer views
        if (hasStructuralMetadata) {
            const ext = this.gltf.extensions?.[EXT_STRUCTURAL_METADATA];
            if (ext?.propertyTables) {
                promises.push(this._loadPropertyTableBuffers(ext.propertyTables));
            }
        }
        ...
        return Promise.all(promises).then(results => {
            ...
            // 准备 structural metadata 数据（供主线程构造插件使用）
            if (hasStructuralMetadata) {
                const metadataData = getStructuralMetadataData(this.gltf, buffers);
                if (metadataData) {
                    gltf.structuralMetadata = metadataData;
                    // 将 buffers 添加到 transferables
                    if (this.transferables) {
                        buffers.forEach(buf => {
            ...
            // EXT_mesh_features 数据已经在 primitive.extensions 中
            // 无需额外处理，主线程可直接使用 primitive.extensions['EXT_mesh_features']
        });
    }

    /**
     * 加载 property tables 所需的 buffer views
     */
    _loadPropertyTableBuffers(propertyTables) {
        const bufferViews = this.gltf.bufferViews || [];
        const bufferViewsToLoad = collectPropertyTableBufferViews(propertyTables);

        // 加载所有需要的 buffer views
        ...
        bufferViewsToLoad.forEach(index => {
            ...
            const promise = this._accessor._requestBufferOfBufferView(bufferView).then(buf => {
                const { buffer, byteOffset } = buf;
                const start = (bufferView.byteOffset || 0) + byteOffset;
                result[index] = buffer.slice(start, start + bufferView.byteLength);
            });
            promises.push(promise);
        });
